...
export default async function handleCustomerPasswordReset({
  ...
  console.log(
    'En handle customer password reset',
    data1.email,
    data1.token,
...
function ResetPasswordEmail({ token, host, mail }: ResetPasswordEmailProps) {
  const base_url = new URL(MEDUSA_ADMIN_BACKEND_URL + '/tienda/reset-password/');
        ...
        <Container style={container}>
          <Section style={coverSection}>
            ...
            <Section style={upperSection}>
              ...
              <Text style={mainText}>
                Entendemos que quieres recuperar tu cuenta, pero debemos verificar que eres el dueño
                de esta cuenta. Por favor ingresa al siguiente link para comprobar que eres el dueño
                de la cuenta y empezar el proceso de reseteo de contraseña. Si no deseas recordar tu
                contraseña, puedes ignorar este mensaje.
              </Text>
              <Section style={verificationSection}>
                <Text style={verifyText}>
                  Pega este código en la app para cambiar tu contraseña
                </Text>

                {/* <Link href={`${base_url.href}`} target="_blank" style={link}>
                  Resetear Contraseña
                </Link> */}
                <Text
                  style={{
